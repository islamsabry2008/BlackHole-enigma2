...
def buildPartitionInfo(partition, partitionList):
	...
	if path.exists(resolveFilename(SCOPE_CURRENT_SKIN, "obhcore/dev_%s.png" % pngType)):
		mypixmap = resolveFilename(SCOPE_CURRENT_SKIN, "obhcore/dev_%s.png" % pngType)
	else:
		mypixmap = "/usr/lib/enigma2/python/Plugins/SystemPlugins/OBH/images/dev_%s.png" % pngType
